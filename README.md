...
**An adversarial platform for AI Integrity: Crucible hunts cases where a model is silently wrong, verifies with independent checks the model and its makers cannot access, hardens the model in a closed loop, and measures its own catch rate against an adversary that already knows the scheme.**
