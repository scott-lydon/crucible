...
def test_engine_finds_all_four_classes():
    ...
    findings = engine.run([AttackClass(c) for c in
                           ["prompt_extraction", "secret_exfil", "tool_abuse", "jailbreak"]])
    found_classes = {f.attack.attack_class for f in findings}
    assert found_classes == set(AttackClass)
    assert all(f.proof.deterministic for f in findings)  # all 4 v1 oracles are ground-truth
