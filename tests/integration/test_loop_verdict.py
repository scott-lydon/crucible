"""Slice 10 wiring: the loop runs the oracles and persists one aggregated verdict.

Drives the orchestrator loop with two deterministic in-test oracles (a PASS
voter and a FAIL voter) over the DummyTarget, so the loop-to-oracles-to-
aggregator-to-Postgres path is exercised on real Postgres with no LLM and no
sandbox. The live five-oracle path is covered by each oracle's own opt-in run.
"""
...
async def test_loop_persists_one_aggregated_verdict(client: AsyncClient) -> None:
